...
| P1.3 | `performDeleteAccount` state cleanup ordering | P1 | IN_PROGRESS | `auth_actions.dart:78-115` | Patch hazır: cleanup (setGuest await + profileController.clear) artık mounted guard ÖNCESİ tamamlanır — yavaş cihazda widget unmount olursa profile state orphan kalmaz. Yeni sıra: deleteAccount → dialog pop → setGuest await → clear → mounted check → snackbar → go. Hata yolu da cleaner: dialog pop → mounted check → error snackbar. `test/account_deletion_p0_test.dart`'a P1.3 order-assertion eklendi (15 test now). Commit/push bekliyor. |
