...
//
// Render: `#$label` text + softGold renk + pill border. `onTap`
...
void main() {
  group('TagChip — render', () {
    testWidgets('Caller ham label → widget içeride "#" prefix ekler',
      ...
      await tester.pumpWidget(_wrap(
        const TagChip(label: 'ekşimaya'),
      ));
      expect(find.text('#ekşimaya'), findsOneWidget);
      expect(find.text('ekşimaya'), findsNothing);
    });

    testWidgets('Label softGold renkte render edilir', (tester) async {
      await tester.pumpWidget(_wrap(
        const TagChip(label: 'simit'),
      ));
      final txt = tester.widget<Text>(find.text('#simit'));
      expect(txt.style?.color, AppColors.softGold);
      // AppTypography.labelLarge -> 13pt w700 letterSpacing 0.4
      expect(txt.style?.fontSize, 13);
      expect(txt.style?.fontWeight, FontWeight.w700);
      expect(txt.style?.letterSpacing, 0.4);
    });
  });
  ...
  group('TagChip — interaction', () {
    ...
    testWidgets('onTap verilirse InkWell + tap callback', (tester) async {
      var taps = 0;
      await tester.pumpWidget(_wrap(
        TagChip(label: 'borek', onTap: () => taps++),
      ));
